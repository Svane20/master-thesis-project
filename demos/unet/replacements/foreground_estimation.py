...
def get_foreground_estimation(image_path: str, alpha_mask: np.ndarray) -> np.ndarray:
    """
    Estimate the foreground of an image.

    Args:
        image_path (str): Path to the image.
        alpha_mask (numpy.ndarray): Continuous alpha mask with values between 0 and 1.
    ...
    return pymatting.estimate_foreground_ml(image=image, alpha=inverted_alpha_mask)
