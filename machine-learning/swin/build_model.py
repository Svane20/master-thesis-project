...
from swin.modeling.meta_arch.swin_small_patch4_window7_224_matte import SwinMattingModel
...
def _build_model(
    ...
    logging.info(f"Building SWIN model in {mode} mode on {device}")
...
def _build(configuration: Dict[str, Any]) -> nn.Module:
    ...
    if configuration["model_name"] == "SwinMattingModel":
