...
def load_config(mode: ConfigurationMode) -> Config:
    return get_configuration(mode=mode, suffix=ConfigurationSuffix.DPT)


def load_config_and_checkpoint_path(mode: ConfigurationMode) -> Tuple[Config, Path]:
    return get_configuration_and_checkpoint_path(mode=mode, suffix=ConfigurationSuffix.DPT)
